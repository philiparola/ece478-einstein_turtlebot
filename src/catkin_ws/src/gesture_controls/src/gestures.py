...
def leftWave():
	print("LEFT")
	shoulder = rospy.Publisher('face_angle', Float64, queue_size=10)
	elbow = rospy.Publisher('face_angle', Float64, queue_size=10)
	wrist = rospy.Publisher('face_angle', Float64, queue_size=10)
	shoulder.publish(resetPos)
	elbow.publish(resetPos)
	wrist.publish(resetPos)
	sleep()
	shoulder.publish(pos)
	elbow.publish(pos)
	wrist.publish(pos)
	sleep()
	shoulder.publish(pos)
